...
SIGNAL_PLUGIN_REF="${SIGNAL_PLUGIN_REF:-v0.27.0}"
...
SPACE="${SPACE:-private-space-121}"
BASE_URL="${BASE_URL:-http://127.0.0.1:18121}"
MESSAGE_MARKER="${MESSAGE_MARKER:-signal-trust-object-store-secret-121}"
...
echo ""
echo "=== Scenario 121 - Signal Trust Object Store ==="
echo ""
...
for required in "backend: object_store" "allow_object_store_backend: true" "type: signal.trust_store" "type: step.signal_trust_observe" "type: step.signal_trust_reset" "type: step.signal_trust_history" "type: step.signal_trust_policy_check" "type: step.signal_encrypt" "type: step.signal_outbox_enqueue"; do
...
if [ "$(jq -r '.snapshot.state.records | length' "$(trust_object_file)" 2>/dev/null)" = "2" ]; then
...
if [ "$(jq -r '.snapshot.state.records | length' "$(trust_object_file)" 2>/dev/null)" = "2" ] &&
   jq -e --arg current "$RESET_RECORD" '.snapshot.state.records[] | select(.record_ref == $current)' "$(trust_object_file)" >/dev/null 2>&1; then
  pass "object trust store snapshot contains the rotated first-pair record and second pair"
else
