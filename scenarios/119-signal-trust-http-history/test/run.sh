...
# step.signal_trust_history, encryption, and outbox enqueue.
set -uo pipefail
export LC_ALL=C
export LANG=C

PLUGIN_NAME="workflow-plugin-signal"
SIGNAL_PLUGIN_REF="${SIGNAL_PLUGIN_REF:-v0.25.0}"
...
SPACE="${SPACE:-private-space-119}"
BASE_URL="${BASE_URL:-http://127.0.0.1:18119}"
TRUST_BACKEND_URL="${TRUST_BACKEND_URL:-http://127.0.0.1:19119}"
SIGNAL_TRUST_HTTP_TOKEN="${SIGNAL_TRUST_HTTP_TOKEN:-scenario-119-token}"
MESSAGE_MARKER="${MESSAGE_MARKER:-signal-trust-http-history-secret-119}"
...
echo ""
echo "=== Scenario 119 - Signal HTTP Trust History ==="
echo ""
...
for required in "backend: http" "type: signal.trust_store" "type: step.signal_trust_observe" "type: step.signal_trust_reset" "type: step.signal_trust_history" "type: step.signal_encrypt" "type: step.signal_outbox_enqueue"; do
...
if [ "$SECOND_STATUS" = "new_trust" ] && [ -n "$SECOND_RECORD" ]; then
  pass "second pair created an independent trust record"
else
...
if printf '%s' "$HISTORY_RESPONSE" | grep -Fq "$SENDER_KEY" ||
   ...
   printf '%s' "$HISTORY_RESPONSE" | grep -Eq 'display|scannable|private_key|plaintext|Authorization|scenario-119-token'; then
...
if [ "$POST_RESTART_STATUS" = "trusted" ] && [ "$POST_RESTART_RECORD" = "$RESET_RECORD" ]; then
  pass "restart preserved rotated trust record from HTTP backend"
else
