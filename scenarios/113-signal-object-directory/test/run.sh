#!/usr/bin/env bash
# Scenario 113 - Signal Object Directory.
#
...
# bundle into the plugin's local object-store public directory backend through
# an operator HTTP route, restarts Workflow against the same object store, then
...
SIGNAL_PLUGIN_REF="${SIGNAL_PLUGIN_REF:-v0.18.0}"
...
BASE_URL="${BASE_URL:-http://127.0.0.1:18113}"
MESSAGE_MARKER="${MESSAGE_MARKER:-signal-public-intake-message-secret-113}"
BLOB_MARKER="${BLOB_MARKER:-signal-public-intake-blob-secret-113}"
BLOB_REF="${BLOB_REF:-blob://scenario-113/private-intake.json}"
...
object_store_files() {
  find "$OBJECT_DIRECTORY_ROOT" -type f -name '*.json' ! -name '*.tmp' | sort
}

echo ""
echo "=== Scenario 113 - Signal Object Directory ==="
echo ""
...
OBJECT_DIRECTORY_ROOT="$DATA_DIR/object-directory"
RUNTIME_CONFIG="$DATA_DIR/app.yaml"
if sed "s#__OBJECT_DIRECTORY_ROOT__#$OBJECT_DIRECTORY_ROOT#g" "$CONFIG" >"$RUNTIME_CONFIG"; then
  pass "generated Workflow app config with per-run object-store root"
else
...
OBJECT_COUNT="$(object_store_files | wc -l | tr -d ' ')"
DIRECTORY_OBJECT_FILE="$(object_store_files | head -n 1)"
if [ "$OBJECT_COUNT" = "1" ] && [ -s "$DIRECTORY_OBJECT_FILE" ]; then
  pass "object-store public directory wrote one persisted object"
else
...
if jq -e --arg intake "$INTAKE_REF" --arg hash "$PUBLISHED_HASH" '
  .backend == "object_store" and
  .intake_ref == $intake and
  (.checksum | startswith("sha256:")) and
  .entry.bundle_sha256 == $hash and
  (.entry.identity_key_fingerprint | length) > 0 and
  .entry.public_metadata.intake_ref == $intake
' "$DIRECTORY_OBJECT_FILE" >/dev/null 2>&1; then
  pass "object-store entry contains checksum, hash, fingerprint, and metadata"
else
...
if start_server "restart"; then
  pass "workflow server restarted against existing object-store directory"
else
  fail "workflow server did not restart against existing object-store directory; see $SERVER_LOG"
  finish
  exit 1
fi
RESTART_RESOLVED="$(curl -fsS "$BASE_URL/intake/$INTAKE_REF/bundle?audience_ref=$AUDIENCE_REF&requested_at_unix=1783000060")" \
  && pass "public caller resolved object-store bundle after restart" \
...
if [ -n "$RESTART_HASH" ] && [ "$RESTART_HASH" = "$PUBLISHED_HASH" ]; then
  pass "restart resolve returned the originally object-stored bundle hash"
else
...
BLOB_TEXT="$(jq -cn --arg marker "$BLOB_MARKER" --arg caller "$CALLER_B" '{kind:"blob",marker:$marker,caller:$caller,rows:[{label:"intake",amount:113}]}')"
...
BLOB_BODY="$(jq -cn \
  ...
  --arg aad_context "scenario://113/signal/public-intake" \
...
DIRECTORY_OBJECT_FILE="$(object_store_files | head -n 1)"
TAMPERED_OBJECT="$DATA_DIR/tampered-directory-object.json"
if jq '.intake_ref = "intake://tampered-object"' "$DIRECTORY_OBJECT_FILE" >"$TAMPERED_OBJECT" &&
   mv "$TAMPERED_OBJECT" "$DIRECTORY_OBJECT_FILE"; then
  pass "tampered object-store directory object on disk"
else
  fail "could not tamper object-store directory object"
fi
stop_server
if start_server "tampered-object"; then
  fail "workflow server accepted tampered object-store directory"
  stop_server
else
  pass "workflow server rejected tampered object-store directory on restart"
fi
