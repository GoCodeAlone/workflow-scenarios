#!/usr/bin/env bash
# Scenario 111 - Signal Public Directory File.
#
...
# bundle into the file-backed public directory through an operator HTTP route,
# restarts against the same directory file, then drives public caller HTTP
# routes for encrypted message/blob submission before operator decrypt.
set -uo pipefail
export LC_ALL=C
export LANG=C

PLUGIN_NAME="workflow-plugin-signal"
SIGNAL_PLUGIN_REF="${SIGNAL_PLUGIN_REF:-v0.16.0}"
...
BASE_URL="${BASE_URL:-http://127.0.0.1:18111}"
MESSAGE_MARKER="${MESSAGE_MARKER:-signal-public-intake-message-secret-111}"
BLOB_MARKER="${BLOB_MARKER:-signal-public-intake-blob-secret-111}"
BLOB_REF="${BLOB_REF:-blob://scenario-111/private-intake.json}"
...
echo ""
echo "=== Scenario 111 - Signal Public Directory File ==="
echo ""
...
if grep -q 'backend: file' "$CONFIG" &&
   grep -q 'storage_path: __PUBLIC_DIRECTORY_PATH__' "$CONFIG" &&
   grep -q 'allow_file_backend: true' "$CONFIG"; then
  pass "Workflow app config declares explicit file-backed public directory"
else
  fail "Workflow app config does not declare explicit file-backed public directory"
fi
...
PUBLIC_DIRECTORY_FILE="$DATA_DIR/public-directory/public-prekeys.json"
RUNTIME_CONFIG="$DATA_DIR/app.yaml"
if sed "s#__PUBLIC_DIRECTORY_PATH__#$PUBLIC_DIRECTORY_FILE#g" "$CONFIG" >"$RUNTIME_CONFIG"; then
  pass "generated Workflow app config with per-run public directory file"
else
...
if [ -s "$PUBLIC_DIRECTORY_FILE" ]; then
  pass "file-backed public directory snapshot was written"
else
  fail "file-backed public directory snapshot was not written"
fi
if jq -e --arg intake "$INTAKE_REF" --arg hash "$PUBLISHED_HASH" '
  .schema_version == 1 and
  .backend == "file" and
  (.checksum | startswith("sha256:")) and
  .state.entries[$intake].bundle_sha256 == $hash and
  (.state.entries[$intake].identity_key_fingerprint | length) > 0 and
  .state.entries[$intake].public_metadata.intake_ref == $intake
' "$PUBLIC_DIRECTORY_FILE" >/dev/null 2>&1; then
  pass "public directory file snapshot contains schema, checksum, hash, and metadata"
else
  fail "public directory file snapshot missing expected persisted evidence"
fi
for forbidden in "$MESSAGE_MARKER" "$BLOB_MARKER" 'custody://' 'credential://' 'operator://host-intake' 'private-key' 'plaintext'; do
  if grep -q "$forbidden" "$PUBLIC_DIRECTORY_FILE"; then
    fail "public directory file leaked forbidden marker $forbidden"
  else
    pass "public directory file did not leak $forbidden"
  fi
done

stop_server
if start_server "restart"; then
  pass "workflow server restarted against existing public directory file"
else
  fail "workflow server did not restart against existing public directory file; see $SERVER_LOG"
  finish
  exit 1
fi
RESTART_RESOLVED="$(curl -fsS "$BASE_URL/intake/$INTAKE_REF/bundle?audience_ref=$AUDIENCE_REF&requested_at_unix=1783000060")" \
  && pass "public caller resolved persisted bundle after restart" \
...
if [ -n "$RESTART_HASH" ] && [ "$RESTART_HASH" = "$PUBLISHED_HASH" ]; then
  pass "restart resolve returned the originally persisted bundle hash"
else
...
BLOB_TEXT="$(jq -cn --arg marker "$BLOB_MARKER" --arg caller "$CALLER_B" '{kind:"blob",marker:$marker,caller:$caller,rows:[{label:"intake",amount:111}]}')"
...
BLOB_BODY="$(jq -cn \
  ...
  --arg aad_context "scenario://111/signal/public-intake" \
...
OBJECT_FILE="$OBJECT_STORE/scenario-111-object.json"
...
stop_server
printf '{not-json' >"$PUBLIC_DIRECTORY_FILE"
if start_server "corrupt"; then
  fail "workflow server accepted corrupt public directory file snapshot"
  stop_server
else
  pass "workflow server rejected corrupt public directory file snapshot"
fi
if grep -Eiq 'public directory file|public pre-key directory|decode' "$SERVER_LOG"; then
  pass "corrupt snapshot rejection was reported by real server/plugin startup"
else
  fail "corrupt snapshot rejection log did not mention public directory failure; see $SERVER_LOG"
fi
