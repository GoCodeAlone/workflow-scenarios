...
The scenario builds `workflow-plugin-signal` v0.14.1, loads it as an external
...
all request inputs. The fake transport path also sends a caller-provided
scheme-shaped raw credential (`foo://...`) to prove the plugin only echoes
known host-managed reference schemes and redacts unknown credential-shaped
inputs.
...
`GoCodeAlone/workflow-plugin-signal` at `SIGNAL_PLUGIN_REF` (default `v0.14.1`).
