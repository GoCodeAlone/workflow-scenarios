...
export SIGNAL_TRANSPORT_HMAC="${SIGNAL_TRANSPORT_HMAC:-scenario-130-hmac-secret}"
SENDER="${SENDER:-user-a}"
RECIPIENT="${RECIPIENT:-user-b}"
WORKER="${WORKER:-signal-transport-worker}"
SPACE="${SPACE:-private-space-130}"
PLAINTEXT_B64="${PLAINTEXT_B64:-c2lnbmFsIGV2ZW50YnVzIHRyYW5zcG9ydCBwcm9vZiAxMzA=}"
MESSAGE_REF="${MESSAGE_REF:-scenario-130-envelope-1}"
TRANSPORT_REF="${TRANSPORT_REF:-transport://scenario/130/eventbus}"
SUBJECT_REF="${SUBJECT_REF:-subject://$SPACE}"
BASE_URL="${BASE_URL:-http://127.0.0.1:18130}"
NATS_ADDR="${NATS_ADDR:-127.0.0.1:19130}"
NATS_HEALTH_ADDR="${NATS_HEALTH_ADDR:-127.0.0.1:19131}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCENARIO_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
REPO_ROOT="$(cd "$SCENARIO_DIR/../.." && pwd)"
CONFIG="$SCENARIO_DIR/config/app.yaml"
BODY_FILE="${TMPDIR:-/tmp}/scenario-130-http-body-$$"
...
echo ""
echo "=== Scenario 130 - Signal Eventbus Transport ==="
echo ""
...
for step_type in step.signal_outbox_enqueue step.signal_outbox_handoff step.eventbus.publish step.eventbus.consume step.signal_transport_admit step.signal_outbox_ack step.eventbus.ack step.signal_inbox_decrypt; do
...
HANDOFF_BODY="$(jq -cn \
  ...
  --arg lease_id "scenario-130-lease-1" \
  --arg transport_ref "$TRANSPORT_REF" \
  --arg subject_ref "$SUBJECT_REF" \
  --arg space "$SPACE" \
  --arg delivery_id "delivery://scenario-130/$MESSAGE_REF" \
