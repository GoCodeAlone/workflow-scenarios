...
Local-only Workflow app proof that `workflow-plugin-signal` v0.36.0 can hand
...
- a second two-message path executes `step.signal_outbox_handoff_batch` and
  `step.signal_transport_admit_batch`, then rejects duplicate admit and route
  mismatch requests through the running Workflow API
...
```bash
bash scenarios/139-signal-batch-transport-lifecycle/test/run.sh
```
