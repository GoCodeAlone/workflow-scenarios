...
# Test script for Scenario 27: Platform Kubernetes
# Tests plan/apply/status/destroy lifecycle using the in-memory kind backend.

NS="${NAMESPACE:-wf-scenario-27}"
PORT=18027
...
echo ""
echo "=== Scenario 27: Platform Kubernetes (in-memory kind backend) ==="
echo ""
...
# ====================================================================
if echo "$RESP" | grep -q "27-platform-kubernetes"; then
    pass "Health check identifies scenario 27-platform-kubernetes"
else
...
# ====================================================================
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/v1/clusters/plan" \
    -H "Content-Type: application/json" -d '{}' 2>/dev/null || echo "000")
if [ "$HTTP_CODE" = "200" ]; then
    pass "POST /api/v1/clusters/plan returns 200"
else
    fail "POST /api/v1/clusters/plan returned $HTTP_CODE (expected 200)"
fi

# ====================================================================
# Test 4: Plan response contains actions
# ====================================================================
PLAN_RESP=$(curl -sf -X POST "$BASE/api/v1/clusters/plan" \
...
# ====================================================================
# Test 5: Plan action type is 'create' (cluster is pending)
# ====================================================================
...
# ====================================================================
# Test 7: Apply returns 200
# ====================================================================
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/v1/clusters/apply" \
    -H "Content-Type: application/json" -d '{}' 2>/dev/null || echo "000")
if [ "$HTTP_CODE" = "200" ]; then
    pass "POST /api/v1/clusters/apply returns 200"
else
    fail "POST /api/v1/clusters/apply returned $HTTP_CODE (expected 200)"
fi

# ====================================================================
# Test 8: Apply response success=true
# ====================================================================
APPLY_RESP=$(curl -sf -X POST "$BASE/api/v1/clusters/apply" \
    -H "Content-Type: application/json" -d '{}' 2>/dev/null || echo "")
...
# ====================================================================
# Test 9: Apply response contains message
# ====================================================================
...
# ====================================================================
# Test 10: Status returns 200
# ====================================================================
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/api/v1/clusters/status" 2>/dev/null || echo "000")
if [ "$HTTP_CODE" = "200" ]; then
    pass "GET /api/v1/clusters/status returns 200"
else
    fail "GET /api/v1/clusters/status returned $HTTP_CODE (expected 200)"
fi

# ====================================================================
# Test 11: Status shows cluster=running after apply
# ====================================================================
STATUS_RESP=$(curl -sf "$BASE/api/v1/clusters/status" 2>/dev/null || echo "")
...
" 2>/dev/null; then
    pass "Status shows cluster status=running after apply"
else
    fail "Status not running after apply: $STATUS_RESP"
fi

# ====================================================================
# Test 12: Status contains cluster name
# ====================================================================
if echo "$STATUS_RESP" | python3 -c "
import json, sys
d = json.load(sys.stdin)
st = d.get('status', {})
assert isinstance(st, dict) and st.get('name'), 'missing name in status'
" 2>/dev/null; then
...
# ====================================================================
# Test 13: Plan after apply returns noop
# ====================================================================
PLAN2_RESP=$(curl -sf -X POST "$BASE/api/v1/clusters/plan" \
    -H "Content-Type: application/json" -d '{}' 2>/dev/null || echo "")
...
# ====================================================================
# Test 14: Destroy returns 200
# ====================================================================
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "$BASE/api/v1/clusters" 2>/dev/null || echo "000")
if [ "$HTTP_CODE" = "200" ]; then
    pass "DELETE /api/v1/clusters returns 200"
else
    fail "DELETE /api/v1/clusters returned $HTTP_CODE (expected 200)"
fi

# ====================================================================
# Test 15: Destroy response destroyed=true
# ====================================================================
DESTROY_RESP=$(curl -sf -X DELETE "$BASE/api/v1/clusters" 2>/dev/null || echo "")
...
# ====================================================================
# Test 16: Status shows deleted after destroy
# ====================================================================
STATUS2_RESP=$(curl -sf "$BASE/api/v1/clusters/status" 2>/dev/null || echo "")
...
" 2>/dev/null; then
    pass "Status shows cluster status=deleted after destroy"
else
