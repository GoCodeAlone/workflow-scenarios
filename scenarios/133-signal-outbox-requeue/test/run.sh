...
# envelope store, drives dead-letter and operator requeue behavior via HTTP,
# restarts the server, and proves recovered delivery state persists.
set -uo pipefail
export LC_ALL=C
export LANG=C
...
WORKER="${WORKER:-signal-requeue-worker}"
OPERATOR="${OPERATOR:-signal-recovery-operator}"
SPACE="${SPACE:-private-space-133}"
PLAINTEXT_B64="${PLAINTEXT_B64:-c2lnbmFsIG91dGJveCByZXF1ZXVlIHByb29mIDEzMw==}"
BASE_URL="${BASE_URL:-http://127.0.0.1:18133}"
RAW_ERROR="raw downstream failure with bearer-token"
SAFE_REASON="reason://scenario/133/invalid-route"
REQUEUE_REASON="reason://scenario/133/operator-retry"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCENARIO_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
REPO_ROOT="$(cd "$SCENARIO_DIR/../.." && pwd)"
CONFIG_TEMPLATE="$SCENARIO_DIR/config/app.yaml"
BODY_FILE="${TMPDIR:-/tmp}/scenario-133-http-body-$$"
...
plugin_repo_supports_requeue() {
  ...
  jq -e '
    ...
    (.capabilities.stepTypes | index("step.signal_outbox_requeue")) and
...
build_plugin() {
  ...
  if [ -z "$plugin_repo" ] || ! plugin_repo_supports_requeue "$plugin_repo"; then
...
for step_type in step.signal_outbox_enqueue step.signal_outbox_claim step.signal_outbox_handoff step.signal_outbox_release step.signal_outbox_dead_letter step.signal_outbox_requeue step.signal_outbox_ack step.signal_envelope_status; do
...
REQUEUED="$(post_json "/operators/$OPERATOR/outbox/requeue" "$(jq -cn --arg envelope_ref "$EXPLICIT_REF" --arg reason "$REQUEUE_REASON" '{envelope_ref:$envelope_ref,reason_ref:$reason,clear_last_error:true,requested_at_unix:3600}')")" \
  && pass "dead-lettered envelope requeued through Workflow API" || fail "requeue failed"
[ "$(printf '%s' "$REQUEUED" | jq -r '.status // empty')" = "queued" ] && pass "requeue returned queued status" || fail "requeue status mismatch: $REQUEUED"
[ "$(printf '%s' "$REQUEUED" | jq -r '.metadata.requeue_count // 0')" = "1" ] && pass "requeue metadata incremented count" || fail "requeue count mismatch: $REQUEUED"
[ "$(printf '%s' "$REQUEUED" | jq -r '.metadata.last_requeued_by_ref // empty')" = "operator://$OPERATOR" ] && pass "requeue metadata recorded operator ref" || fail "requeue operator mismatch: $REQUEUED"
[ "$(printf '%s' "$REQUEUED" | jq -r '.metadata.last_requeue_reason_ref // empty')" = "$REQUEUE_REASON" ] && pass "requeue metadata recorded reason ref" || fail "requeue reason mismatch: $REQUEUED"
[ "$(printf '%s' "$REQUEUED" | jq -r '.metadata.reason_ref // empty')" = "" ] && pass "requeue cleared stale dead-letter reason" || fail "requeue leaked stale reason: $REQUEUED"
assert_no_marker "requeue response" "$REQUEUED" "$PLAINTEXT_B64"
assert_no_marker "requeue response" "$REQUEUED" "$EXPLICIT_LEASE"

RECOVERY_CLAIM="$(post_json "/workers/$WORKER/outbox/claim" "$(jq -cn --arg envelope_ref "$EXPLICIT_REF" '{envelope_ref:$envelope_ref,lease_id:"after-requeue",requested_at_unix:3700}')")" \
  && pass "requeued envelope claimed after recovery" || fail "claim after requeue failed"
RECOVERY_LEASE="$(printf '%s' "$RECOVERY_CLAIM" | jq -r '.lease_ref // empty')"
[ "$(printf '%s' "$RECOVERY_CLAIM" | jq -r '.status // empty')" = "claimed" ] && pass "claim after requeue returned claimed" || fail "claim after requeue mismatch: $RECOVERY_CLAIM"
assert_no_marker "claim after requeue" "$RECOVERY_CLAIM" "$PLAINTEXT_B64"

ACKED="$(post_json "/workers/$WORKER/outbox/ack" "$(jq -cn --arg envelope_ref "$EXPLICIT_REF" --arg lease_ref "$RECOVERY_LEASE" '{envelope_ref:$envelope_ref,lease_ref:$lease_ref,requested_at_unix:3800}')")" \
  && pass "requeued envelope acked after recovery" || fail "ack after requeue failed"
[ "$(printf '%s' "$ACKED" | jq -r '.status // empty')" = "acked" ] && pass "ack after requeue returned acked" || fail "ack after requeue mismatch: $ACKED"
BAD_REQUEUE_CODE="$(post_status "/operators/$OPERATOR/outbox/requeue" "$(jq -cn --arg envelope_ref "$EXPLICIT_REF" --arg reason "$REQUEUE_REASON" '{envelope_ref:$envelope_ref,reason_ref:$reason,clear_last_error:true,requested_at_unix:3900}')")"
assert_rejected_not_missing "requeue of acked envelope" "$BAD_REQUEUE_CODE"
...
if printf '%s' "$SNAPSHOT" | grep -Fq 'acked' && printf '%s' "$SNAPSHOT" | grep -Fq 'requeue_count'; then
  pass "SQLite snapshot contains recovered ack and requeue lineage"
else
  fail "SQLite snapshot did not contain recovered ack/requeue lineage"
fi
if printf '%s' "$SNAPSHOT" | grep -Fq "$REQUEUE_REASON"; then
  pass "SQLite snapshot contains safe requeue reason"
else
  fail "SQLite snapshot did not contain safe requeue reason"
fi
...
[ "$(printf '%s' "$RESTART_STATUS" | jq -r '.items[0].status // empty')" = "acked" ] && pass "recovered ack status persisted after restart" || fail "restart status mismatch: $RESTART_STATUS"
[ "$(printf '%s' "$RESTART_STATUS" | jq -r '.items[0].requeue_count // 0')" = "1" ] && pass "requeue count persisted after restart" || fail "restart requeue count mismatch: $RESTART_STATUS"
[ "$(printf '%s' "$RESTART_STATUS" | jq -r '.items[0].last_requeue_reason_ref // empty')" = "$REQUEUE_REASON" ] && pass "safe requeue reason persisted after restart" || fail "restart requeue reason mismatch: $RESTART_STATUS"
[ "$(printf '%s' "$RESTART_STATUS" | jq -r '.items[0].reason_ref // empty')" = "" ] && pass "dead-letter reason stayed cleared after restart" || fail "restart leaked stale dead-letter reason: $RESTART_STATUS"
