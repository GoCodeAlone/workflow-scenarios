#!/usr/bin/env bash
# Scenario 125 - Signal envelope object-store persistence.
#
# Demonstration-fidelity: this starts the real Workflow server with an
# object_store signal.envelope_store, drives separate sender/worker/recipient
# API calls, restarts the app, and proves queued, released, and acked state
# survives through the plugin's object-store backend.
set -uo pipefail
export LC_ALL=C
export LANG=C
...
SPACE="${SPACE:-private-space-125}"
BASE_URL="${BASE_URL:-http://127.0.0.1:18125}"
PLAINTEXT_QUEUED_B64="${PLAINTEXT_QUEUED_B64:-c2lnbmFsIG9iamVjdCBxdWV1ZWQgcHJvb2YgMTI1}"
PLAINTEXT_RELEASED_B64="${PLAINTEXT_RELEASED_B64:-c2lnbmFsIG9iamVjdCByZWxlYXNlZCBwcm9vZiAxMjU=}"
PLAINTEXT_ACKED_B64="${PLAINTEXT_ACKED_B64:-c2lnbmFsIG9iamVjdCBhY2tlZCBwcm9vZiAxMjU=}"
...
plugin_repo_supports_object_lifecycle() {
  local repo="$1"
  ...
  grep -q 'allow_object_store_backend' "$repo/internal/contracts/signal.proto" 2>/dev/null
}
...
build_plugin() {
  ...
  if [ -z "$plugin_repo" ] || ! plugin_repo_supports_object_lifecycle "$plugin_repo"; then
...
post_status() {
  local path="$1"
  local body="$2"
  curl -sS -o /tmp/scenario-125-http-body -w "%{http_code}" \
...
release_envelope() {
  ...
  body="$(jq -cn --arg envelope_ref "$envelope_ref" --arg lease_ref "$lease_ref" \
    '{envelope_ref:$envelope_ref,lease_ref:$lease_ref,last_error_ref:"error://scenario-125/transient"}')"
...
echo ""
echo "=== Scenario 125 - Signal Envelope Object Store ==="
echo ""
...
for marker in 'backend: object_store' 'allow_object_store_backend: true' 'type: step.signal_outbox_release' 'type: step.signal_outbox_ack'; do
...
OBJECT_STORE_PATH="$DATA_DIR/object-store"
mkdir -p "$OBJECT_STORE_PATH"
RUNTIME_CONFIG="$DATA_DIR/app.yaml"
sed "s#__OBJECT_STORE_PATH__#$OBJECT_STORE_PATH#g" "$CONFIG_TEMPLATE" >"$RUNTIME_CONFIG"
...
if start_server; then
  pass "workflow server started with object-store envelope store"
else
...
QUEUED_SENT="$(send_envelope scenario-125-queued "$PLAINTEXT_QUEUED_B64")" \
  && pass "queued envelope was stored in object-backed outbox" \
  || fail "queued envelope send failed"
QUEUED_REF="$(printf '%s' "$QUEUED_SENT" | jq -r '.envelope_ref // empty')"

RELEASED_SENT="$(send_envelope scenario-125-released "$PLAINTEXT_RELEASED_B64")" \
  && pass "released envelope was stored in object-backed outbox" \
  || fail "released envelope send failed"
RELEASED_REF="$(printf '%s' "$RELEASED_SENT" | jq -r '.envelope_ref // empty')"
RELEASED_CLAIM="$(claim_envelope "$RELEASED_REF" scenario-125-release-lease)" \
...
[ "$(printf '%s' "$RELEASED_RELEASE" | jq -r '.metadata.last_error_ref // empty')" = "error://scenario-125/transient" ] && pass "release wrote ref-only error metadata" || fail "release metadata mismatch: $RELEASED_RELEASE"

ACKED_SENT="$(send_envelope scenario-125-acked "$PLAINTEXT_ACKED_B64")" \
  && pass "acked envelope was stored in object-backed outbox" \
  || fail "acked envelope send failed"
ACKED_REF="$(printf '%s' "$ACKED_SENT" | jq -r '.envelope_ref // empty')"
ACKED_CLAIM="$(claim_envelope "$ACKED_REF" scenario-125-ack-lease)" \
...
OBJECT_SNAPSHOT="$(find "$OBJECT_STORE_PATH/envelopes" -name snapshot.json -type f | head -n 1)"
if [ -s "$OBJECT_SNAPSHOT" ]; then
  pass "object-store snapshot exists"
else
  fail "object-store snapshot was not created"
fi

if jq -e '.backend == "object_store" and .store_ref == "signal_envelopes" and (.snapshot.checksum | startswith("sha256:"))' "$OBJECT_SNAPSHOT" >/dev/null 2>&1; then
  pass "object-store snapshot has backend, store ref, and checksum"
else
  fail "object-store snapshot metadata missing or invalid"
fi
STATE_JSON="$(jq -c '.snapshot.state // empty' "$OBJECT_SNAPSHOT" 2>/dev/null)"
for plaintext in "$PLAINTEXT_QUEUED_B64" "$PLAINTEXT_RELEASED_B64" "$PLAINTEXT_ACKED_B64"; do
  if printf '%s' "$STATE_JSON" | grep -q "$plaintext"; then
    fail "object-store snapshot leaked plaintext marker $plaintext"
  else
    pass "object-store snapshot did not expose plaintext marker $plaintext"
  fi
done
if printf '%s' "$STATE_JSON" | grep -q 'ciphertext'; then
  pass "object-store snapshot contains ciphertext envelope state"
else
  fail "object-store snapshot did not contain ciphertext envelope state"
fi

stop_server
OBJECT_SNAPSHOT_BACKUP="$OBJECT_SNAPSHOT.good"
cp "$OBJECT_SNAPSHOT" "$OBJECT_SNAPSHOT_BACKUP"
jq '.snapshot.checksum = "sha256:bad"' "$OBJECT_SNAPSHOT_BACKUP" >"$OBJECT_SNAPSHOT"
if start_server; then
  fail "object-store accepted tampered checksum on startup"
  stop_server
else
  pass "object-store rejected tampered checksum on startup"
fi
cp "$OBJECT_SNAPSHOT_BACKUP" "$OBJECT_SNAPSHOT"
mkdir -p "$OBJECT_STORE_PATH/envelopes/wrong-key"
cp "$OBJECT_SNAPSHOT_BACKUP" "$OBJECT_STORE_PATH/envelopes/wrong-key/snapshot.json"
if start_server; then
  fail "object-store accepted wrong object key on startup"
  stop_server
else
  pass "object-store rejected wrong object key on startup"
fi
rm -rf "$OBJECT_STORE_PATH/envelopes/wrong-key"
if start_server; then
  pass "workflow server restarted against the same object-store envelope store"
else
...
QUEUED_CLAIM="$(claim_envelope "$QUEUED_REF" scenario-125-queued-after-restart)" \
...
RELEASED_RECLAIM="$(claim_envelope "$RELEASED_REF" scenario-125-released-after-restart)" \
...
[ "$(printf '%s' "$RELEASED_RECLAIM" | jq -r '.metadata.last_error_ref // empty')" = "error://scenario-125/transient" ] && pass "released error ref survived restart" || fail "released error ref mismatch: $RELEASED_RECLAIM"
...
ACKED_RECLAIM_BODY="$(jq -cn --arg envelope_ref "$ACKED_REF" '{envelope_ref:$envelope_ref,lease_id:"scenario-125-acked-after-restart"}')"
ACKED_RECLAIM_STATUS="$(post_status "/workers/$WORKER/outbox/claim" "$ACKED_RECLAIM_BODY")"
[ "$ACKED_RECLAIM_STATUS" != "200" ] && pass "acked envelope stayed terminal across restart" || fail "acked envelope was reclaimable after restart: $(cat /tmp/scenario-125-http-body)"
