#!/usr/bin/env bash
# Scenario 124 - Signal envelope SQLite persistence.
#
# Demonstration-fidelity: this starts the real Workflow server with a SQLite
...
WORKER="${WORKER:-sqlite-worker}"
SPACE="${SPACE:-private-space-124}"
BASE_URL="${BASE_URL:-http://127.0.0.1:18124}"
PLAINTEXT_QUEUED_B64="${PLAINTEXT_QUEUED_B64:-c2lnbmFsIHNxbGl0ZSBxdWV1ZWQgcHJvb2YgMTI0}"
PLAINTEXT_RELEASED_B64="${PLAINTEXT_RELEASED_B64:-c2lnbmFsIHNxbGl0ZSByZWxlYXNlZCBwcm9vZiAxMjQ=}"
PLAINTEXT_ACKED_B64="${PLAINTEXT_ACKED_B64:-c2lnbmFsIHNxbGl0ZSBhY2tlZCBwcm9vZiAxMjQ=}"
...
plugin_repo_supports_sqlite_lifecycle() {
  local repo="$1"
  ...
  grep -q 'allow_sqlite_backend' "$repo/internal/contracts/signal.proto" 2>/dev/null
}
...
build_plugin() {
  ...
  if [ -z "$plugin_repo" ] || ! plugin_repo_supports_sqlite_lifecycle "$plugin_repo"; then
...
post_status() {
  local path="$1"
  local body="$2"
  curl -sS -o /tmp/scenario-124-http-body -w "%{http_code}" \
...
release_envelope() {
  ...
  body="$(jq -cn --arg envelope_ref "$envelope_ref" --arg lease_ref "$lease_ref" \
    '{envelope_ref:$envelope_ref,lease_ref:$lease_ref,last_error_ref:"error://scenario-124/transient"}')"
...
echo ""
echo "=== Scenario 124 - Signal Envelope SQLite ==="
echo ""
...
for marker in 'backend: sqlite' 'allow_sqlite_backend: true' 'type: step.signal_outbox_release' 'type: step.signal_outbox_ack'; do
...
SQLITE_STORE_PATH="$DATA_DIR/signal/envelopes.db"
mkdir -p "$(dirname "$SQLITE_STORE_PATH")"
RUNTIME_CONFIG="$DATA_DIR/app.yaml"
sed "s#__SQLITE_STORE_PATH__#$SQLITE_STORE_PATH#g" "$CONFIG_TEMPLATE" >"$RUNTIME_CONFIG"
...
if start_server; then
  pass "workflow server started with SQLite envelope store"
else
...
QUEUED_SENT="$(send_envelope scenario-124-queued "$PLAINTEXT_QUEUED_B64")" \
  && pass "queued envelope was stored in SQLite-backed outbox" \
  || fail "queued envelope send failed"
QUEUED_REF="$(printf '%s' "$QUEUED_SENT" | jq -r '.envelope_ref // empty')"

RELEASED_SENT="$(send_envelope scenario-124-released "$PLAINTEXT_RELEASED_B64")" \
  && pass "released envelope was stored in SQLite-backed outbox" \
  || fail "released envelope send failed"
RELEASED_REF="$(printf '%s' "$RELEASED_SENT" | jq -r '.envelope_ref // empty')"
RELEASED_CLAIM="$(claim_envelope "$RELEASED_REF" scenario-124-release-lease)" \
...
[ "$(printf '%s' "$RELEASED_RELEASE" | jq -r '.metadata.last_error_ref // empty')" = "error://scenario-124/transient" ] && pass "release wrote ref-only error metadata" || fail "release metadata mismatch: $RELEASED_RELEASE"

ACKED_SENT="$(send_envelope scenario-124-acked "$PLAINTEXT_ACKED_B64")" \
  && pass "acked envelope was stored in SQLite-backed outbox" \
  || fail "acked envelope send failed"
ACKED_REF="$(printf '%s' "$ACKED_SENT" | jq -r '.envelope_ref // empty')"
ACKED_CLAIM="$(claim_envelope "$ACKED_REF" scenario-124-ack-lease)" \
...
if [ -s "$SQLITE_STORE_PATH" ]; then
  pass "SQLite database file exists"
else
  fail "SQLite database file was not created"
fi

STATE_JSON="$(sqlite3 "$SQLITE_STORE_PATH" 'SELECT state_json FROM signal_envelope_store_snapshots WHERE store_ref = "signal_envelopes";' 2>/dev/null)"
if [ -n "$STATE_JSON" ]; then
  pass "SQLite snapshot row exists"
else
  fail "SQLite snapshot row missing"
fi
for plaintext in "$PLAINTEXT_QUEUED_B64" "$PLAINTEXT_RELEASED_B64" "$PLAINTEXT_ACKED_B64"; do
  if printf '%s' "$STATE_JSON" | grep -q "$plaintext"; then
    fail "SQLite snapshot leaked plaintext marker $plaintext"
  else
    pass "SQLite snapshot did not expose plaintext marker $plaintext"
  fi
done
if printf '%s' "$STATE_JSON" | grep -q 'ciphertext'; then
  pass "SQLite snapshot contains ciphertext envelope state"
else
  fail "SQLite snapshot did not contain ciphertext envelope state"
fi

stop_server
if start_server; then
  pass "workflow server restarted against the same SQLite envelope store"
else
...
QUEUED_CLAIM="$(claim_envelope "$QUEUED_REF" scenario-124-queued-after-restart)" \
...
RELEASED_RECLAIM="$(claim_envelope "$RELEASED_REF" scenario-124-released-after-restart)" \
...
[ "$(printf '%s' "$RELEASED_RECLAIM" | jq -r '.metadata.last_error_ref // empty')" = "error://scenario-124/transient" ] && pass "released error ref survived restart" || fail "released error ref mismatch: $RELEASED_RECLAIM"
...
ACKED_RECLAIM_BODY="$(jq -cn --arg envelope_ref "$ACKED_REF" '{envelope_ref:$envelope_ref,lease_id:"scenario-124-acked-after-restart"}')"
ACKED_RECLAIM_STATUS="$(post_status "/workers/$WORKER/outbox/claim" "$ACKED_RECLAIM_BODY")"
[ "$ACKED_RECLAIM_STATUS" != "200" ] && pass "acked envelope stayed terminal across restart" || fail "acked envelope was reclaimable after restart: $(cat /tmp/scenario-124-http-body)"
